...
$assignment_query = "SELECT id, assignment_title, close_datetime, max_score, assignment_instruction, submission_type, isPublished FROM assignments WHERE id = " . $assignment_id;
                            ...
                            <div class="flex-col mx-20">
                                <div class="p-5 text-justify" style="position: relative; margin-bottom: 15px">
                                    ...
                                    <div class="left-align quiz-header">
                                        ...
                                        <!-- START BUTTON -->
                                        <?php
                                        if (($current_time >= strtotime($assignment['isPublished']) && $current_time <= strtotime($assignment['close_datetime'])) || $assignment["close_datetime"] == NULL)
                                            echo '
                                            <div class="t-center">
                                    ...
                                    <?php if (($current_time >= strtotime($assignment['isPublished']) && $current_time <= strtotime($assignment['close_datetime'])) || $assignment["close_datetime"] == NULL): ?>
                                        <!-- INSTRUCTIONS -->
                                        <p class="text-justify"><?= $assignment["assignment_instruction"] ?></p>
                                    <?php else: ?> 
                                        <div class="t-center">
                                            <img src="../images/lock.png" alt="lock" class="logo">
                                        </div>
                                        <p class="t-center">Assignment is locked until <?= date("F d, Y h:i A", strtotime($assignment['isPublished'])) ?></p>
                                    <?php endif ?>
                                </div>
                            </div>
                            <br>
