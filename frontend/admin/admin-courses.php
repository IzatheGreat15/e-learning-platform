...
if (!isset($_GET['mode'])) {
    ...
    if (isset($_GET['sid']))
        $course = mysqli_fetch_assoc($db->query("SELECT subjects.* FROM subject_group JOIN subjects ON subject_group.subject_id = subjects.id WHERE subject_group.id = " . $_GET['sid']));
}
?>
            ...
            <div class="content full-width white">
                ...
                <div class="flex flex-col" style="margin-top: 5%;">
                    <form action="<?= !isset($_GET['id']) ? "../../backend/admin/create_subjects.php" : "../../backend/admin/update_subjects.php" ?>" class="flex-col mx-20" method="POST">
                        ...
                        <input type="text" class="border-bottom px-10" name="title" placeholder="Subject" value="<?= isset($_GET['id']) ? $course['subject_name'] : '' ?>" <?= $isTeacherR ?>>
                        ...
                        <select name="teacher" id="" class="white rounded-corners px-10" <?= $isTeacherD ?>>
                                ...
                                <table class="whole">
                                    <?php foreach ($db->query("SELECT * FROM subject_group WHERE subject_id = " . $_GET['id']) as $sg) : ?>
                                        ...
                                        <tr class="t-center sg pointer" id="<?= $sg['id'] ?>">
                                            <td style="width: 25%;">
                                                <p><?= $sg['subject_group_name'] ?></p>
                                            </td>
                                            <td style="width: 25%;">
                                                <p>Grade <?= $section['year_level'] ?> - Section <?= $section['section_name'] ?></p>
                                            </td>
                                            <td style="width: 25%;">
                                                <p>Teacher <?= isset($teacher) ? $teacher['fname'] . ' ' . $teacher['lname'] : '' ?></p>
                                            </td>
                                            <td style="width: 25%;">
                                                <p><?= $sg['schedule'] != NULL ? $sg['schedule'] : "Schedule Unset" ?></p>
                                            </td>
                                        </tr>
                                    <?php endforeach ?>
                                </table>
